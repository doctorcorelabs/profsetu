// Netlify Scheduled Function - Auto RSS Update 2x sehari
...
const handler = schedule('0 6,18 * * *', async (event) => {
  ...
  try {
    ...
    console.log(`✅ RSS processing completed: ${successCount}/${RSS_FEEDS.length} feeds, ${totalProcessed} items`);
    
    return {
      statusCode: 200,
      body: JSON.stringify({ 
        message: 'RSS scheduled processing completed successfully',
    ...
    console.error('❌ RSS scheduled processing failed:', error);
    return {
      statusCode: 500,
      body: JSON.stringify({ 
        error: 'RSS scheduled processing failed',
